...
async def refresh_user(user_id: int = Path(ge=1, description="Enter User ID", example=1),
...
async def delete_user(user_id: int = Path(ge=1, description="Enter User ID", example=1)):
    for user in users:
        if user.id == user_id:
            users.remove(user)
            return user
...
#uvicorn Unik2.main2:app --reload